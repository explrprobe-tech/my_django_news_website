...
class EditNewsViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client.login(username=self.editor_user.username, password=self.editor_password)
    @with_fresh_news
    def test_edit_news_view_get(self):
    ...
    @with_fresh_news
    def test_edit_news_view_post_valid(self):
        update_data = {
    ...
    @with_fresh_news
    def test_edit_news_view_post_invalid(self):
        update_data = {
            'title': 'Title after change',
            'content': 'Content after change',
            'category': self.category_biology.pk
        ...
        for field in required_fields:
            ...
            del update_data_copy[field]        
            response = self.client.post(reverse('edit_news', kwargs={'pk': self.created_news.pk}), data=update_data_copy)
            self.assertEqual(response.status_code, 200, f'Created news was edited with invalid field {field}')
            self.assertIn(field, response.context['form'].errors, f'Missed field {field} is not in error list')
        none_required_fields = ['content']
        for field in none_required_fields:
            update_data_copy = update_data.copy()
            del update_data_copy[field]
