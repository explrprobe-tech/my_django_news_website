...
class AuthTest(TestCase):
    ...
    def test_edit_news_permision(self):
        self.client.logout()
        anonymous_user_response = self.client.get(reverse('edit_news', kwargs={'pk': self.created_news.pk}))
        self.assertEqual(anonymous_user_response.status_code, 302, 'Anonymous user should not have access to edit news')
        self.assertEqual('/login/?next=/news/1/edit/', anonymous_user_response.url, 'Anonymous user does not have redirect url')
        self.client.login(username=self.regular_user.username, password=self.regular_password)
        regular_user_response = self.client.get(reverse('edit_news', kwargs={'pk': self.created_news.pk}))
        self.assertEqual(regular_user_response.status_code, 403, 'Regular user should not have access to edit news')
        self.client.logout()
        self.client.login(username=self.admin_user.username, password=self.admin_password)
        admin_user_response = self.client.get(reverse('edit_news', kwargs={'pk': self.created_news.pk}))
        self.assertEqual(admin_user_response.status_code, 200, 'Admin user should have access to edit news')
        self.client.logout()
        self.client.login(username=self.editor_user.username, password=self.editor_password)
        edit_user_response = self.client.get(reverse('edit_news', kwargs={'pk': self.created_news.pk}))
        self.assertEqual(edit_user_response.status_code, 200, 'Admin user should have access to edit news')
    def test_category_creation_permission(self):
        self.client.logout()
        annonymous_user_response = self.client.get(reverse('add_category'))
        self.assertEqual(annonymous_user_response.status_code, 302, 'Anonymous user should not have access to add category')
        self.client.login(username=self.regular_user, password=self.regular_password)
        regular_user_response = self.client.get(reverse('add_category'))
        self.assertEqual(regular_user_response.status_code, 403, 'Regular user should not have access to add category')
        self.client.logout()
        self.client.login(username=self.admin_user, password=self.admin_password)
        admin_user_response = self.client.get(reverse('add_category'))
        self.assertEqual(admin_user_response.status_code, 200, 'Admin user should have access to add category')
        self.client.logout()
        self.client.login(username=self.editor_user, password=self.editor_password)
        editor_user_response = self.client.get(reverse('add_category'))
        self.assertEqual(editor_user_response.status_code, 200, 'Editor user should have access to add category')
    def test_category_deletion_permision(self):
        category_test = Category.objects.create(title='Test Category')
        self.client.logout()
        annonymous_user_response = self.client.post(reverse('category_delete', kwargs={'pk': category_test.pk}))
        self.assertEqual(annonymous_user_response.status_code, 302, 'Anonymous user should not have access to delete category')
        self.client.login(username=self.regular_user, password=self.regular_password)
        regular_user_response = self.client.post(reverse('category_delete', kwargs={'pk': category_test.pk}))
        self.assertEqual(regular_user_response.status_code, 302, 'Regular user should not have access to delete category')
        self.client.logout()
        self.client.login(username=self.admin_user, password=self.admin_password)
        admin_user_response = self.client.post(reverse('category_delete', kwargs={'pk': category_test.pk}))
        self.assertEqual(admin_user_response.status_code, 302, 'Admin user should have access to delete category')
        self.client.logout()
        category_test = Category.objects.create(title='Test Category')
        self.client.login(username=self.editor_user, password=self.editor_password)
